import imgLeadership1 from 'assets/images/Leadership/img_leadership1.png';
import { dataTabsLeadershipType } from 'components/templates/Leadership';

const dataLeadershipList: dataTabsLeadershipType[] = [
  {
    titleTab: 'Hội đồng quản trị',
    dataTab: [
      {
        gender: 'Ông',
        name: 'Bùi Thành Nhơn',
        position: 'Nhà sáng lập - Chủ tịch Hội đồng Quản trị',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Ông',
        name: 'Bùi Xuân Huy',
        position: 'Thành viên Hội đồng Quản trị, Tổng Giám đốc',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Bà',
        name: 'Hoàng Thu Châu',
        position: 'Thành viên Hội đồng Quản trị, Phó Tổng Giám đốc',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Ông',
        name: 'Phạm Tiến Vân',
        position: 'Thành viên Hội đồng Quản trị Độc lập',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Ông',
        name: 'Lê Quốc Hùng',
        position: 'Thành viên Hội đồng Quản trị Độc lập',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Bà',
        name: 'Nguyễn Mỹ Hạnh',
        position: 'Thành viên Hội đồng Quản trị Độc lập',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
    ],
  },
  {
    titleTab: 'Ban giám đốc',
    dataTab: [
      {
        gender: 'Ông',
        name: 'Bùi Thành Nhơn',
        position: 'Nhà sáng lập - Chủ tịch Hội đồng Quản trị',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Ông',
        name: 'Bùi Xuân Huy',
        position: 'Thành viên Hội đồng Quản trị, Tổng Giám đốc',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Bà',
        name: 'Hoàng Thu Châu',
        position: 'Thành viên Hội đồng Quản trị, Phó Tổng Giám đốc',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Ông',
        name: 'Phạm Tiến Vân',
        position: 'Thành viên Hội đồng Quản trị Độc lập',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Ông',
        name: 'Lê Quốc Hùng',
        position: 'Thành viên Hội đồng Quản trị Độc lập',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
      {
        gender: 'Bà',
        name: 'Nguyễn Mỹ Hạnh',
        position: 'Thành viên Hội đồng Quản trị Độc lập',
        imgSrc: imgLeadership1,
        achievement: [
          'Cử nhân Nông nghiệp. Tốt nghiệp Executive MBA, HSB-TUCK, Đại học Dartmouth, Hoa Kỳ.',
          'Là một trong những chủ doanh nghiệp tư nhân đầu tiên tại TP.HCM từ những năm 1980. Thành viên Tổ chức doanh nhân thế giới (YPO - WPO).',
        ],
        slogan: 'Con người Novaland với giá trị cốt lõi: Hiệu quả - Chính trực - Chuyên nghiệp là tài sản quý báu của Tập đoàn; là chìa khóa của sự thành công sẽ sớm hiện thực được Sứ mạng, Hoài bão của Novaland Group.',
      },
    ],
  },
];

export default dataLeadershipList;
